...
(function ($) {
  ...
  $(window).on("scroll", function () {
    ...
    if (customStickyElements.length > 0) {
      let customStickyElementsOffset =
        customStickyElements.offset().top - sectionHeaderHeight;
...
//   }
  ...
  if ($(".exp-text-animate-1").length > 0) {
    ...
    let subTitleContainer = new SplitText(".exp-text-animate-1", {
      type: "chars",
    });
  ...
  //   function textAnimate(sliderElement) {
  //     const textsToAnimate = sliderElement.querySelectorAll(
  //       ".exp-text-animate-3"
  //     );
  //     textsToAnimate.forEach((textToAnimate) => {
  ...
  function changeBackgroundImage(heroImages, heroWrapper, currentImageIndex) {
